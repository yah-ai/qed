...
/// @yah:verify("Independently re-verified 2026-09-13 by @Ashguard:libra (session:a5287a05), third confirmation after the implementer and @Ashguard:eclipse. scripts/check-yah-index-live.sh against the LIVE CDN object: 5 passed / 0 failed (9193 bytes; no legacy bare digest in any triple of any version; every hash blake3:-tagged and every bootstrap_hash sha256:-tagged; 5 versions newest-first each with pub_date + manifest_url + >=1 download; cache-control: no-cache, max-age=0). `cargo test -p yah-qed --lib` from oss/qed: 984 passed / 1 failed / 1 ignored — the single failure is still `tests::desktop_release_matrix_routes_each_row_to_its_own_platform`, confirmed by name, the same pre-existing unrelated failure as the 979 and 982 baselines. Count drifted 982 -> 984 from peers' tests landing in the shared tree, not from this ticket.")
/// @yah:gotcha("SUPERSEDED (2026-09-13): the prior gotcha said this ticket was stuck at `handoff` because review refused it. It is now at `review` — `mcp__yah__board_review {\"id\": \"R330-T52\", \"verify\": [...]}` succeeded first try from a session rooted at /Users/leif/ss/yah, no --path, no retry, despite the ticket being anchored at oss/qed/crates/qed/src/publish.rs:343. So the `conflict: ticket not found` R511-B7 records is NOT a blanket property of oss/-anchored tickets; whatever it is, it did not reproduce here. Data point added to R511-B7.")
/// @yah:verify("GAP IN MY 2026-09-13 RE-VERIFICATION, stated so a reviewer does not read it as wider than it is: I could NOT re-run `cargo test -p yah --lib qed_publish` (the 14/0 baseline). The `yah` crate does not compile in this tree for a reason unrelated to this ticket — R876-B16 added `secrets: Vec<SecretMount>` to `MesofactRevalidateReceiver` (oss/yah-base/crates/workload-spec/src/lib.rs:1439) and `MesofactRevalidateReceiverSpec::to_workload_payload` at oss/yubaba/crates/cloud/src/reconciler/mesofact_bundle.rs:595 has not been updated, so E0063. Reproduced twice ~10 min apart, so not a stale-artifact flake. I did not add an inert `secrets: vec![]`: @Miravel:libra is live on R876 and that field exists to carry real materialized mounts, so a vec![] would compile and be wrong. Nudged them on party.chat. That suite was verified twice already (implementer + @Ashguard:eclipse) and a peer's struct-field addition cannot regress it, so this is blocked-not-failing — but it is unre-run, and the live-CDN check plus yah-qed --lib are what my confirmation actually rests on.")
/// @yah:verify("GAP ABOVE IS NOW CLOSED — read the two entries together. @Miravel:libra landed R876-B16's remaining call sites (to_workload_payload now takes a real `secrets: Vec<SecretMount>` param, cloud.rs's production caller included), so the E0063 is gone and I re-ran what was blocked. `cargo test -p yah --lib qed_publish`: 14 passed / 0 failed, EXACTLY the 14/0 baseline — every named test present, including normalizing_the_live_object_removes_the_16_bare_digests_and_nothing_else, a_rewrite_that_changes_anything_but_the_forbidden_keys_is_refused, the_repair_writes_once_and_is_then_a_no_op, a_racing_publisher_is_remerged_rather_than_lost, and merging_the_next_release_preserves_all_five_historical_entries. R330's own relay verify `cargo check -p yah`: Finished clean, warnings only. Separately: the camp skew detector flagged oss/qed/crates/qed/src/publish.rs — this ticket's own file — as modified mid-run, so I re-read it. All three legs still wired: `strip_legacy_digests(&mut prior)` at publish.rs:450 inside merge_index, the free fn at :464, the normalize_index strip at :493, `index_legacy_bare_digest_paths` at :508, and all four test call sites at :1329/:1345/:1386/:1432. Nothing was disturbed. All four of this ticket's named baselines are now independently re-confirmed rather than three of four.")
pub fn merge_index(
    existing: Option<&str>,
    binary: &str,
    version: &str,
