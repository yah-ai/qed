...
//! @yah:handoff("DONE. manual no longer parses-and-rejects: doc_source.rs lowers it to R622's StepKind::Manual + ManualConfig. The cell body's LEADING comment block is the human's brief - bare prose becomes prompt, '# advance:' the verifiable condition, '# checklist:' an advisory box; everything below the block becomes terminal prefill the form shows and never runs. A '#' line below the commands stays an ordinary shell comment. advance stays optional, for W257's BIOS block exactly. on_fail is always Abort, never the assert/show choice, because continuing past a gate a person just refused is not a thing a runbook should be able to express.")
//! @yah:handoff("THE LOAD-BEARING RULE IS STRUCTURAL, NOT CONVENTIONAL. The hole was real and I found it: subagent.answer_ask (camp.rs) submits ANY form by id, and a QED park echoes its form_id on StepAwaitingHuman, which any agent reads back off qed.status - so an agent could answer its own gate. Now QedFormManualGate mints the park form with metadata.prefer='human' (R469's routing hint), and agent_may_answer() refuses such a form outright before submit_form. R469 documented prefer as advisory; this makes it a rule on the one surface that bypasses routing, which incidentally closes the same hole for ask_user{prefer:human}. Still not a permissions system (W282 non-goal): it does not ask WHICH human, only that the answer did not come from an agent answering by id.")
//! @yah:next("NOT DONE, genuinely separable: cells.json (R717-F6) has no notion of a PARKED cell. camp.rs persists a non-terminal run on StepAwaitingHuman (R622), so a parked cell can fold into the index as status 'running' and 'yah qed doc' renders it that way. A distinct awaiting-human CELL state wants a StepStatus-level spelling, which is R622's type to grow rather than this ticket's.")
//! @yah:handoff("REJECTED AT PARSE, each with its own error naming the cell: manual+assert (no argv means no exit code to make a verdict of), manual+capture (nothing ever writes $YAH_OUTPUTS), manual+host (a person at a keyboard is not a remote command - put the ssh in '# advance:', which is where W296's router-reservation cell wants it anyway), a manual cell with no prompt, a blank '# advance:', and two '# advance:' lines. DocSourceError::ManualNotYetSupported is gone.")
//! @yah:handoff("DISCOVERED WORK, done in this pass. (a) CellFence.tsx (R717-F9) DISABLED the run button on every manual cell with the title 'blocked on R622' - now that manual lowers, that left the operator no way to open their own gate; button enabled, title reads 'Park <id> on a form in the AnswerQueue'. (b) cellInfo.ts's FenceCellAttrs.manual doc said 'the runner REJECTS it today', which my change disproved; rewritten. (c) 'yah qed doc' marked manual rows [manual] in the table and manual:true in --json - the reader of that table is often the agent that just found out it cannot advance the doc. (d) qed.run's description in BOTH agent-tools/qed_tools.rs and mcp/tools.rs now says a manual cell is a human's gate you cannot answer (well inside the ~4pct catalog budget headroom; schema_drift and the budget ratchet both green).")
//! @yah:verify("cargo test -p yah-qed --lib: 823 pass / 1 fail. The failure is PRE-EXISTING and unrelated: tests::desktop_release_matrix_routes_each_row_to_its_own_platform, whose panic names the two Linux rows another session removed from .yah/qed/desktop-release.toml.")
//! @yah:gotcha("TWO PRE-EXISTING RED TESTS, neither from this work, both confirmed by reading their panic text. (1) camp::r325_f1_tests::release_wizard_composes_and_gates_on_advance - 'manual step roll-the-fleet has no advance'; already filed as R741-B1 (open) at .yah/qed/release-wizard.toml:70, blocked on an ops call about which command proves fleet convergence. (2) yah-qed tests::desktop_release_matrix_routes_each_row_to_its_own_platform - the recipe lost its two Linux matrix rows. Neither is anywhere near cell lowering.")
//! @yah:verify("THE TICKET'S OWN VERIFY, as a test: runner::tests::a_doc_manual_cell_parks_the_run_and_nothing_downstream_executes. Parses a doc with W257's BIOS cell (manual, no advance) followed by an assert cell that touches a marker, lowers it, runs it against a gate that parks and never answers. Asserts the run does NOT resolve inside 600ms, StepAwaitingHuman carries the cell id and a form id, the park request carries the parsed prompt+checklist, the gate step never finishes, the next cell never starts, and the marker file does not exist.")
//! @yah:verify("cargo test -p yah --lib r325_f1: 36 pass / 1 fail (the R741-B1 one above), incl. the new a_qed_manual_park_mints_a_form_no_agent_may_answer - the park form declares a human actor and agent_may_answer refuses it, while an ordinary ask_user form with no prefer still routes to an ancestor.")
//! @yah:cleanup("NOT COMPILED: the desktop crate (app/yah/desktop). Its type-check was denied at the approval gate, so I did not run it. Inference, not a verified claim: DocCell gained a field and yah_qed gained a ManualCell re-export, and desktop/src/qed.rs constructs no DocCell (only its own DocCellStateWire), so nothing there should break - but somebody should run it.")
//! @yah:next("PATHSPEC (T11 only): oss/qed/crates/qed/src/{doc_source,types,runner,lib}.rs app/yah/cli/src/{camp.rs,qed.rs,mcp/tools.rs} crates/yah/agent-tools/src/qed_tools.rs packages/yah/ui/src/components/agent/messages/fences/{CellFence.tsx,cellInfo.ts} .yah/docs/working/W296-executable-docs-notebook-cells.md")
//! @yah:verify("cargo test -p xtask --test schema_drift: 3 pass (ManualConfig is untouched; only a private helper's visibility changed, so no schema moved). cd packages/yah/ui && bun test src/components/agent/messages/fences: 30 pass / 0 fail; bun run typecheck clean. cargo clippy -p yah-qed --lib: zero warnings in doc_source.rs.")
