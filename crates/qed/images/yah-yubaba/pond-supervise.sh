...
#
# Environment knobs:
#   KAMAJI_SOCK   UDS path; default /run/kamaji/kamaji.sock
...
KAMAJI_SOCK="${KAMAJI_SOCK:-/run/kamaji/kamaji.sock}"
export RUST_LOG="${YAH_LOG:-${RUST_LOG:-info}}"

mkdir -p "$(dirname "$KAMAJI_SOCK")"
# A stale socket from a prior container life would block kamaji's bind.
rm -f "$KAMAJI_SOCK"

echo "[yah-yubaba] starting kamaji on $KAMAJI_SOCK" >&2
/usr/local/bin/kamaji --socket "$KAMAJI_SOCK" &
constable_pid=$!

# Wait for kamaji to bind the UDS before yubaba tries to dial in. Bounded
# so we fail loudly if kamaji can't start.
for _ in $(seq 1 50); do
    if [[ -S "$KAMAJI_SOCK" ]]; then
        break
    fi
    sleep 0.1
done
if [[ ! -S "$KAMAJI_SOCK" ]]; then
    echo "[yah-yubaba] kamaji did not bind $KAMAJI_SOCK within 5s; aborting" >&2
    kill "$constable_pid" 2>/dev/null || true
    exit 1
fi

echo "[yah-yubaba] starting yah-yubaba serve (KAMAJI_SOCK=$KAMAJI_SOCK)" >&2
...
KAMAJI_SOCK="$KAMAJI_SOCK" /usr/local/bin/yubaba serve ${YAH_WARDEN_ARGS:-} &
warden_pid=$!
